...
; Version 1.0.8 0 21-11-2017 Tweaks for CLI
; ---------------------------------------------------------------------------------------
(define (script-fu-CarTOONizeCLI file_prefix)
...
;	 (file_prefix "frame")
	 (out_prefix "out-")
         ...
         (Prefilter TRUE)                                  ; Noise reduction Prefilter (TRUE, FALSE)
         (filter-type 0) ; Noise Reduction Filter type ("Selective Gaussian Blur Smoothing" "Despeckle Smoothing" "GREYCStoration Smoothing" "G'MIC Anistropic Smoothing")
         (line-size 10)                                    ; Line Thickness in pixels (10 2 30 1 10 0 1)
         (enable-shadow TRUE)                              ; Black Shadows (TRUE FALSE)
         (shadow-intensity 35)                             ; Shadow Intensity (35 0 50 1 10 0 1)
         (cdepth 4)                                        ; Color Levels (4 1 5 1 10 0 1)
         (new-layer FALSE)                                 ; Generate as new layer (FALSE, TRUE)
         (half-tone FALSE)                                 ; Halftone (FALSE, TRUE)
    ...
    (for-each 
      (lambda (filename)
        (let* (
          ...
          (img (car (gimp-file-load RUN-NONINTERACTIVE filename filename)))  ; load the image
          ...
          (layer-cartoonize (car (gimp-layer-new img width height 0 "CarTOONizeCLI"  100 NORMAL-MODE)))
          ...
          (gimp-message "started undo group on img.  about to start on toon-img")
          ...
          (gimp-message "made some temp layers.")
          ...
          (gimp-message "should we prefilter with grey?")
          (if (= Prefilter TRUE)
           (if (= filter-type 2)
          ...
          (gimp-message "should we prefilter with gmic?")
          (if (= Prefilter TRUE)
           (if (= filter-type 3)
            (if (= isfiltered FALSE)
          ...
          (gimp-message "should we prefilter with selective gauss?")
          (if (= Prefilter TRUE)
           (if (= filter-type 0)
            (if (= isfiltered FALSE)
         ...
         (gimp-file-save RUN-NONINTERACTIVE img drawable destination_file destination_file)
         (gimp-image-delete img)                  ; unload the image
        )
      )
      varFileList
    )
